...
<el-dialog :visible.sync="dialogVisible" :before-close="closeDialog" width="800px" custom-class="course-dialog" v-on:open="openDialog">
	<el-form ref="tmpCourse" :model="tmpCourse" label-width="180px" :rules="rules" v-on:validate="validate">
		<el-form-item label="Course" prop="name">
			<el-input v-model.trim="tmpCourse.name"></el-input>
		</el-form-item>
		
		<el-form-item label="Awarding Organization" prop="awardingOrganization">
			<el-input v-model.trim="tmpCourse.awardingOrganization"></el-input>
		</el-form-item>
		...
		<el-form-item label="Training Hours" prop="trainingHours">
			<el-input v-model.trim="tmpCourse.trainingHours"></el-input>
		</el-form-item>
		
		<el-form-item label="Course Fee" prop="courseFee">
			<el-input v-model.trim="tmpCourse.courseFee"></el-input>
			<span> / Person</span>
		</el-form-item>
		...
		<el-form-item label="Link to skill frame work" prop="dutyLevelSkillCode">
			<el-select v-model="tmpCourse.dutyLevelSkillCode" popper-class="training-skill-select">
				<el-option v-for="item in subCategory" :label="item.name" :value="item.code" :key="item.id"/>
			</el-select>
		</el-form-item>
	</el-form>
	
	<div style="margin:45px auto 10px auto;" :style="{width: type==0?'408px':type==1?'522px':'280px' }">
		<el-button v-on:click="handleCancel" :disabled="disabled">Cancel</el-button>
		<el-button v-if="type!=2" type="success" v-on:click="handleSave" :disabled="disabled">Save as Plan</el-button>
		<el-button type="success" v-on:click="handleConfirm" :disabled="disabled">Add as Confirmed</el-button>
		<el-button v-if="type==1" type="danger" v-on:click="handleDelete" :disabled="disabled">Delete Plan</el-button>
	</div>
		...
		return{
			...
			planRules:{
                ...
                name:[
                    ...
                    { min: 1, max: 50, message: 'name length more than 50', trigger: 'blur' }
                ],
                awardingOrganization:[
                    { min: 1, max: 50, message: 'organization length more than 50', trigger: 'blur' }
                ],
                members:[
